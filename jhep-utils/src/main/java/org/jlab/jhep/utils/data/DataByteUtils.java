...
public class DataByteUtils {
    ...
    public static TreeMap<Integer,Integer> bitMap = DataByteUtils.createBitMap();
    ...
    public static byte[] ungzip(final byte[] gzipped) {
        ...
        try {
            ...
            final byte[] buffer = new byte[DataByteUtils.MTU];
            int bytesRead = 0;
            while (bytesRead != -1) {
                bytesRead = inputStream.read(buffer, 0, DataByteUtils.MTU);
                if (bytesRead != -1) {
    ...
    public static int getInteger(int data, int bitstart, int bitend){
        ...
        if(DataByteUtils.bitMap.containsKey(length)==true){
            int value = ((data>>bitstart)&DataByteUtils.bitMap.get(length));
            return value;
        } else {
    ...
    public static int write(final int word, int number, int start, int end){
        ...
        int result = (word & ~(DataByteUtils.bitMap.get(index)<<(start)));
        result = result|( (number&DataByteUtils.bitMap.get(index))<<(start)) ;
        return result;
    }
    /**
    ...
    public static long writeLong(final long word, int number, int start, int end){
        int index   = end - start + 1;
        long result = (word & ~(DataByteUtils.bitMap.get(index)<<(start)));
        result = result|( (number&DataByteUtils.bitMap.get(index))<<(start)) ;
        return result;
    }
    /**
    ...
    public static int read(final int word, int start, int end){
        int index = end-start+1;
        return (word>>(start))&DataByteUtils.bitMap.get(index);
    }
    ...
    public static int readLong(final long word, int start, int end){
        int index = end-start+1;
        return (int) ((word>>(start))&DataByteUtils.bitMap.get(index));
    }
    ...
    public static void main(String[] args){
        ...
        int headerM = DataByteUtils.write(header, 4, 10, 12);
        int headerR = DataByteUtils.read(headerM, 10,12);
        
        System.out.println(DataByteUtils.getByteString(header));
        System.out.println(DataByteUtils.getByteString(headerM));
        System.out.println(DataByteUtils.getByteString(headerR));
        
        byte[] buffer = DataByteUtils.generateByteArray(1845000);
        System.out.println("Length = " + buffer.length);
        byte[] compressed = DataByteUtils.gzip(buffer);
        System.out.println("compressed length = " + compressed.length);
        
        long stime_ = System.currentTimeMillis();
        for(int loop = 0; loop < 50000; loop++){
            byte[] dc = DataByteUtils.ungzip(compressed);
        }
