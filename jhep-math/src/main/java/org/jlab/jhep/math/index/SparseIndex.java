...
import org.jlab.jhep.utils.data.DataByteUtils;
...
public class SparseIndex {
    ...
    public long getKey(int [] bin){        
        ...
        long key = 0;        
        for (int iaxis = 0; iaxis < rank; iaxis++){
            key = DataByteUtils.writeLong(key,bin[iaxis],offset[iaxis],offset[iaxis]+bitsPerAxis[iaxis]);
        }
        return key;
    }
    /**
    ...
    public long getKeyFast(int [] bin){        
        long key = 0;        
        for (int axis = 0; axis < rank; axis++){
            key = DataByteUtils.writeLong(key,bin[axis],offset[axis],offset[axis]+bitsPerAxis[axis]);
        }
        return key;
    }
    ...
    public void getIndex(long key, int[] bin) {       
        int index = 0; 
        for (int iaxis = 0; iaxis < rank; iaxis++){
            bin[iaxis] = DataByteUtils.readLong(key, index, index+bitsPerAxis[iaxis]-1);
    ...
    public String toIndexString(long key){
        int[] indicies = new int[this.getRank()]; 
        this.getIndex(key, indicies);
        StringBuilder str = new StringBuilder();
        str.append(" { ");
        for(int i = 0; i < indicies.length; i++){
            if(i!=0) str.append(",");
            str.append(String.format("%5d", indicies[i]));
        }
        str.append(" }");
        return str.toString();
    }
