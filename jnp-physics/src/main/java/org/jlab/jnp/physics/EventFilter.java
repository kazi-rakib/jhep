...
public class EventFilter {
    ...
    public boolean checkFinalState(PhysicsEvent event)
    {
        ...
        int pc = event.getParticleList().countByCharge(+1);
        int pm = event.getParticleList().countByCharge(-1);
        int pn = event.getParticleList().countByCharge( 0);
        ...
        while(it.hasNext())
        {
            ...
            if(event.getParticleList().countByPid(key)<idcount) return false;
        }
        return filter;
    }
